...
def get_db_name(db_name):
    return db_name or envs.MONGO_DATABASE_NAME


def get_collection_name(collection):
    return collection or envs.MONGO_COLLECTION_DATA_NAME


class Connect:
    connection = None
    db_name = envs.MONGO_DATABASE_NAME

    @classmethod
    def get_connection(
        cls,
        connection_string=envs.MONGO_CONNECTION_STRING,
        connection_timeout=envs.MONGO_CONNECT_TIMEOUT,
        min_pool_size=envs.MONGO_MIN_POOL_SIZE,
        max_pool_size=envs.MONGO_MAX_POOL_SIZE,
        write_concern_level=envs.MONGO_WRITE_CONCERN_LEVEL,
        read_concern_level=envs.MONGO_READ_CONCERN_LEVEL,
    ):
...
def get_collection(collection, db_name=None):
    ...
    return Connect.get_connection().get_database(db_name).get_collection(collection)
...
def create_timeseries_collection(
    ...
    db_conn = Connect.get_connection().get_database(db_name)

    try:
        db_conn.create_collection(name=collection_name, timeseries=timeseries_config)
