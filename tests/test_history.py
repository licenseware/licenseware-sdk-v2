...
class TestHistory(unittest.TestCase):
    def test_uploader_builder(self):
        ...
        def processing_function_without_decorator(
            filepath, event_id, uploader_id, tenant_id
        ):
            """Getting some data out of provided cpuq.txt file"""
            print(f"Processing {filepath}...")

            history.add_entities(
                event_id, entities=[str(uuid.uuid4()), str(uuid.uuid4())]
            )
            # do something else
            entity_to_remove = str(uuid.uuid4())
            history.add_entities(
                event_id,
                entities=[entity_to_remove, str(uuid.uuid4()), str(uuid.uuid4())],
            )
